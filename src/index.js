...
// ---------- CLI (reads input.json like your Python) ----------
if (import.meta.url === `file://${__filename}`) {
  const cfgPath = path.join(process.cwd(), "input.json");
  if (!fs.existsSync(cfgPath)) {
    console.error("❌ Missing input.json in current directory.");
    console.error("📝 Create input.json with format:");
    console.error(`{
  ...
  (async () => {
    try {
      console.log("🎙️  Generating announcement...");
      console.log(`📁 Using voices from: ${VOICE_DIR}\n`);

      const { text, audio, extras } = await makeAnnouncement(
        cfg.text,
        cfg.lang,
        cfg.gender,
        cfg.style,
        {
          customStyle: cfg.custom_style ?? null,
          master: cfg.master ?? true,
          exportFormats: cfg.export_formats ?? null,
        }
      );

      console.log("📝 Generated announcement:");
      console.log(`   "${text}"\n`);
      console.log(`🎵 Main audio file: ${audio}`);
      ...
      if (err.message.includes("Missing voice")) {
        console.error("\n💡 Try running: npm run setup-voices");
      }
      process.exit(1);
    }
  })();
